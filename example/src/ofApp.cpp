...
bool ofApp::significantMinima(float a, float b, float c, float sig) {
    return (b + sig < a) && (b + sig < c);
}

void ofApp::drawRgbPlot(unsigned char* data, int size, int offset, int width, int height) {
...
void ofApp::keyPressed(int key) {
    ...
    if(key == 's'){
        pix.setFromPixels(image,scanner.getPixelsPerLine(),scanner.getTotalLines(),3);
